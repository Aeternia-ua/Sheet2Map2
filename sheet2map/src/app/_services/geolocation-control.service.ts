...
export class GeolocationControlService {
  // private controlUI: any;
  btnIsActive = false;
  btnStateChange: Subject<boolean> = new Subject<boolean>();

  constructor(private agmGeolocationService: AgmGeolocationService) {
    this.btnStateChange.subscribe((value) => {
      this.btnIsActive = value;
    });
  }

  toggleClass(): void {
    this.btnStateChange.next(!this.btnIsActive);
  }
